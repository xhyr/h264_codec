...
void MBInterRDOptimizer::Encode()
{
	...
	auto allowed_mb_types = { MBType::P16x16, MBType::P16x8}; 
	for (auto mb_type : allowed_mb_types)
	{
		...
		int64_t rate = Binary(tmp_bytes_data);
		//substract mb skip
		rate -= 1;
...
uint64_t MBInterRDOptimizer::Binary(std::shared_ptr<BytesData> bytes_data)
{
