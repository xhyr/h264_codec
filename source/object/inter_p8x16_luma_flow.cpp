...
uint32_t InterP8x16LumaFlow::OutputMotionInfo(std::shared_ptr<BytesData> bytes_data) const
{
...
void InterP8x16LumaFlow::Reconstruct()
{
	for (uint8_t segment_index = 0; segment_index < 2; ++segment_index)
	{
		...
		auto [start_x, start_y] = SegmentUtil::GetLumaDataRect<8, 16>(segment_index);
		for (uint32_t y = start_y; y < start_y + 16; ++y)
		{
