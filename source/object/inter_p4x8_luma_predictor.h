...
{
public:
	InterP4x8LumaPredictor(std::shared_ptr<Macroblock> mb, std::shared_ptr<EncoderContext> encoder_context, uint8_t segment_index, uint8_t sub_segment_index);
	~InterP4x8LumaPredictor();

	void Decide();

	BlockData<4, 8> GetPredictedData() const;
...
private:
	...
	BlockData<4, 8> m_predicted_data;
	BlockData<4, 8, int32_t> m_diff_data;
	MotionVector m_mvd;
};
